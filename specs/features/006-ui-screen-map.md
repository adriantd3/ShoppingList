...
- FR-ui-02: When authenticated, the app entry shall route to the last active shopping list persisted locally on the client inside the Lists tab.
...
- FR-ui-11: Notifications MVP shall support only relevant push events (share-link accepted, item added with anti-saturation policy, reset executed, and shopping ended) with quiet-hours and dedupe controls.
...
## Acceptance Criteria
- When an authenticated user launches the app and a locally persisted last active list exists, the system shall open that list directly.
- When an authenticated user launches the app and no active list exists, the system shall open Lists tab collection view.
- When the locally persisted last active list is missing or no longer accessible, the system shall open Lists tab collection view.
...
- When push events are emitted for share-link accepted, item added, reset, and shopping ended, the system shall apply quiet-hours deferral and per-event dedupe rules.
